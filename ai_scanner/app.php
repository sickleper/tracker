...
function receiptScannerConfigWarnings(string $openAiKey): array
{
    $warnings = [];
...
function receiptScannerRequireReceiptAiConfig(string $openAiKey): void
{
    if ($openAiKey === '') {
...
function receiptScannerRequireMileageAiConfig(string $openAiKey): void
{
    if ($openAiKey === '') {
        throw new Exception('Mileage scanner is not configured: OPENAI_API_KEY is missing.');
    }
}
...
function receiptScannerStorePublicReceiptRecord(array $record): void
{
    $response = makeApiCall('/api/public/fuel/receipt-scanner/receipts', $record, 'POST');
    if (!is_array($response) || !($response['success'] ?? false)) {
        throw new Exception($response['message'] ?? 'Failed to store public receipt record.');
    }
}
...
function receiptScannerReadJsonlRecords(string $path, int $limit = 10): array
{
...
function receiptScannerPublicRecordMatches(array $record, array $context): bool
{
    $contextUserHash = trim((string) ($context['user_hash'] ?? ''));
    $recordUserHash = trim((string) ($record['user_hash'] ?? ''));
    if ($contextUserHash !== '' && $recordUserHash !== '') {
        return hash_equals($contextUserHash, $recordUserHash);
    }

    $contextUserId = (int) ($context['driver_id'] ?? 0);
    $recordUserId = (int) ($record['user_id'] ?? 0);
    if ($contextUserId > 0 && $recordUserId > 0 && $contextUserId === $recordUserId) {
        return true;
    }
...
function receiptScannerPublicHistory(array $context, string $type, int $limit = 8): array
{
    $path = $type === 'mileage'
        ? ($GLOBALS['mileage_jsonl_file'] ?? '')
        : ($GLOBALS['jsonl_file'] ?? '');

    $records = receiptScannerReadJsonlRecords((string) $path, max(1, $limit * 4));
    $history = [];

    foreach ($records as $record) {
        if (!receiptScannerPublicRecordMatches($record, $context)) {
            continue;
        }
...
function receiptScannerPublicFileUrl(?string $fileName): ?string
{
    ...
    $path = ($GLOBALS['upload_dir'] ?? (__DIR__ . '/uploads/')) . $fileName;
    if (!is_file($path)) {
        return null;
    }
...
function receiptScannerPublicHistoryPayload(array $record, string $type): array
{
    if ($type === 'mileage') {
        return [
            'id' => (string) ($record['id'] ?? ''),
            'created_at' => (string) ($record['created_at'] ?? ''),
            'vehicle_reg' => (string) ($record['vehicle_reg'] ?? ''),
            'mileage' => isset($record['mileage']) ? (int) $record['mileage'] : null,
            'file_name' => (string) ($record['mileage_image'] ?? ''),
            'file_url' => receiptScannerPublicFileUrl($record['mileage_image'] ?? ''),
        ];
    }
    ...
    return [
        ...
        'total_amount' => isset($record['total_amount']) ? (float) $record['total_amount'] : null,
        'category' => (string) ($record['category'] ?? ''),
        'record_status' => (string) ($record['record_status'] ?? ''),
        'needs_office_sync' => !empty($record['needs_office_sync']),
        'file_name' => (string) ($record['receipt_image'] ?? ''),
        'file_url' => receiptScannerPublicFileUrl($record['receipt_image'] ?? ''),
    ];
}
...
$scannerConfigWarnings = array_merge(
    receiptScannerConfigWarnings($openai_key),
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['capture_fuel_log'])) {
    ...
    try {
        ...
        receiptScannerRequireReceiptAiConfig($openai_key);
        receiptScannerRequireMileageAiConfig($openai_key);
        ...
        $receiptData = extractReceiptData($receiptPath, $openai_key);
        $mileageData = extractMileageData($mileagePath, $openai_key);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['receipt'])) {
    ...
    if (move_uploaded_file($_FILES['receipt']['tmp_name'], $target_file)) {
        try {
            receiptScannerRequireApiAccess();
            receiptScannerRequireReceiptAiConfig($openai_key);
            $extracted_data = extractReceiptData($target_file, $openai_key);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'review' && isset($_GET['id'])) {
    header('Content-Type: application/json');
    receiptScannerRequireApiAccess();
    ...
    $api_response = makeApiCall($receipt_api_base . '/' . rawurlencode($id) . '/review', $payload, 'PATCH');
    if (!is_array($api_response)) {
        receiptScannerApiFailure('Failed to update receipt review via API.');
    }
...
function extractReceiptData($image_path, $openai_key) {
    $ocr_text = '';
    $processing_method = 'openai_ocr_gpt4o';
    $mimeType = '';
    $temporaryVisionSource = null;
    try {
        ...
        if ($mimeType === 'application/pdf') {
            $pdfText = receiptScannerExtractPdfTextWithPython($image_path);
            if (trim($pdfText) !== '') {
                $ocr_text = $pdfText;
                $processing_method = 'pdf_text_extract_openai';
            } else {
                $temporaryVisionSource = receiptScannerConvertPdfFirstPageToPng($image_path);
                $ocr_text = receiptScannerExtractOcrWithOpenAiVision($temporaryVisionSource, $openai_key);
                $processing_method = 'pdf_to_image_openai_vision';
            }
        } else {
            $ocr_text = receiptScannerExtractOcrWithOpenAiVision($image_path, $openai_key);
            $processing_method = 'openai_vision';
        }

        if (trim($ocr_text) === '') {
            throw new Exception('No text was extracted from the uploaded file.');
        }
    } catch (Exception $e) {
        throw new Exception('Receipt OCR failed: ' . $e->getMessage());
    }
    
    if ($temporaryVisionSource && is_file($temporaryVisionSource)) {
        @unlink($temporaryVisionSource);
    }
...
function receiptScannerConvertPdfFirstPageToPng(string $pdfPath): string
{
    if (!is_file($pdfPath) || !is_readable($pdfPath)) {
        throw new Exception('PDF file is not readable for conversion.');
    }

    $outputBase = tempnam(sys_get_temp_dir(), 'scanner_pdf_');
    if ($outputBase === false) {
        throw new Exception('Unable to allocate temporary file for PDF conversion.');
    }

    $outputPng = $outputBase . '.png';
    @unlink($outputBase);

    $pythonDepsPath = __DIR__ . '/.pydeps';
...
pdf_path = sys.argv[1]
out_path = sys.argv[2]
deps_path = sys.argv[3] if len(sys.argv) > 3 else ""
errors = []
...
for label, converter in (("PyMuPDF", try_pymupdf), ("pypdfium2", try_pypdfium2)):
    try:
        converter()
        print(label)
        sys.exit(0)
...
PY;
    ...
    $pythonScriptPath = tempnam(sys_get_temp_dir(), 'scanner_py_');
    if ($pythonScriptPath === false) {
        @unlink($outputPng);
        throw new Exception('Unable to create temporary Python script for PDF conversion.');
    }
    ...
    if ($exitCode !== 0) {
        ...
        if ($errorText === '') {
            $errorText = 'No renderer available (expected PyMuPDF or pypdfium2).';
        }
        throw new Exception('PDF to image conversion via Python failed: ' . $errorText);
    }

    if (!is_file($outputPng) || filesize($outputPng) <= 0) {
        throw new Exception('PDF conversion produced no image output.');
    }

    return $outputPng;
}

function receiptScannerExtractPdfTextWithPython(string $pdfPath): string
{
    if (!is_file($pdfPath) || !is_readable($pdfPath)) {
        return '';
    }
...
pdf_path = sys.argv[1]
deps_path = sys.argv[2] if len(sys.argv) > 2 else ""
if deps_path:
    sys.path.insert(0, deps_path)
...
try:
    ...
    for idx in range(max_pages):
        chunks.append((doc.load_page(idx).get_text("text") or "").strip())
    doc.close()
...
PY;
    ...
    $pythonScriptPath = tempnam(sys_get_temp_dir(), 'scanner_pdf_text_');
    if ($pythonScriptPath === false) {
        return '';
    }
...
function receiptScannerExtractOcrWithOpenAiVision(string $imagePath, string $openAiKey): string
{
    if ($openAiKey === '') {
        throw new Exception('OPENAI_API_KEY is missing for OpenAI OCR fallback.');
    }

    $imageContent = @file_get_contents($imagePath);
    if ($imageContent === false) {
        throw new Exception('Failed to read fallback image for OpenAI OCR.');
    }
    ...
    $payload = [
        ...
        'messages' => [
            [
                'role' => 'system',
                'content' => 'You are an OCR assistant. Extract all visible text exactly. Return plain text only.',
            ],
            [
                'role' => 'user',
                'content' => [
                    [
                        'type' => 'text',
                        'text' => 'Extract all text from this document image. Keep line breaks where possible.',
                    ],
                    [
                        'type' => 'image_url',
                        'image_url' => [
    ...
    if ($curlError) {
        throw new Exception('OpenAI OCR fallback connection error: ' . $curlError);
    }

    if ($httpCode !== 200) {
        $errorBody = json_decode((string) $response, true);
        $errorMessage = $errorBody['error']['message'] ?? 'Unknown OpenAI error';
        throw new Exception('OpenAI OCR fallback failed (HTTP ' . $httpCode . '): ' . $errorMessage);
    }

    $result = json_decode((string) $response, true);
    $content = trim((string) ($result['choices'][0]['message']['content'] ?? ''));
    if ($content === '') {
        throw new Exception('OpenAI OCR fallback returned empty text.');
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['mileage_photo']) && $receiptScannerAllowsPublicDriverUpload) {
    ...
    try {
        ...
        $mileage_data = extractMileageData($target_file, $openai_key);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['receipt']) && $receiptScannerAllowsPublicDriverUpload) {
    header('Content-Type: application/json');

    try {
        receiptScannerValidateUploadedFile($_FILES['receipt'], ['image/jpeg', 'image/png', 'application/pdf']);

        $fileName = time() . '_' . preg_replace('/[^A-Za-z0-9._-]/', '_', basename((string) $_FILES['receipt']['name']));
        ...
        $extractedData = extractReceiptData($targetFile, $openai_key);
        $publicUser = !empty($receiptScannerPublicDriverContext['user_hash'])
            ? receiptScannerResolvePublicUserByHash((string) $receiptScannerPublicDriverContext['user_hash'])
            : receiptScannerResolvePublicUserById((int) ($receiptScannerPublicDriverContext['driver_id'] ?? 0));
        ...
        $publicReceipt = [
            ...
            'discount_amount' => $extractedData['discount'],
            'currency' => $extractedData['currency'],
            'receipt_number' => $extractedData['receipt_number'],
            'location' => $extractedData['location'],
            'phone' => $extractedData['phone'],
            'category' => $extractedData['category'],
            'payment_method' => $extractedData['payment'],
            'items' => json_decode($extractedData['items']),
            'receipt_image' => $fileName,
            'confidence_score' => $extractedData['confidence'],
            'processing_method' => $extractedData['processing_method'],
            'raw_ocr_text' => $extractedData['ocr_text'],
            'extracted_json' => $extractedData['extracted_json'],
            'uploaded_by' => trim((string) ($receiptScannerPublicDriverContext['driver_name'] ?? 'Driver Upload Link')),
            'project_address' => $extractedData['project_address'] ?? null,
            'gps_latitude' => $extractedData['gps_latitude'] ?? null,
            'gps_longitude' => $extractedData['gps_longitude'] ?? null,
            'upload_source' => 'driver_whatsapp_link',
            'record_origin' => 'public_worker_upload',
            'record_status' => 'synced_to_api',
            'needs_office_sync' => false,
            'created_at' => date('Y-m-d H:i:s'),
        ];

        receiptScannerStorePublicReceiptRecord($publicReceipt);

        echo json_encode([
            'success' => true,
            'data' => [
                'merchant' => $publicReceipt['merchant_name'],
                'total' => $publicReceipt['total_amount'],
                'date' => $publicReceipt['transaction_date'],
                'category' => $publicReceipt['category'],
                'record_status' => $publicReceipt['record_status'],
            ],
            'message' => 'Receipt uploaded successfully',
        ]);
    } catch (Exception $e) {
        if (!empty($targetFile) && is_file($targetFile)) {
            @unlink($targetFile);
        }
        echo json_encode(['success' => false, 'error' => 'Processing error: ' . $e->getMessage()]);
    }
...
if (isset($_GET['action']) && $_GET['action'] === 'public_history' && $receiptScannerAllowsPublicDriverUpload) {
    header('Content-Type: application/json');

    $type = trim((string) ($_GET['type'] ?? ''));
    if (!in_array($type, ['receipt', 'mileage'], true)) {
        echo json_encode(['success' => false, 'error' => 'Invalid history type.']);
        exit;
    }

    $history = array_map(
        static fn(array $record): array => receiptScannerPublicHistoryPayload($record, $type),
        receiptScannerPublicHistory($receiptScannerPublicDriverContext, $type, 8)
    );
    echo json_encode([
        'success' => true,
        'type' => $type,
...
function extractMileageData($image_path, string $openAiKey = '') {
    try {
        $finfo = finfo_open(FILEINFO_MIME_TYPE);
        $mimeType = finfo_file($finfo, $image_path);
        finfo_close($finfo);
...
$scannerDefaultVehicleReg = strtoupper(trim((string) ($scannerAssignedVehicle['license_plate'] ?? '')));
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<body class="bg-slate-50 text-slate-900 dark:bg-slate-950 dark:text-slate-100">
    <div class="container scanner-layout-shell mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <div class="header mb-6 rounded-[28px] border border-slate-200/70 bg-white/90 px-6 py-7 text-left shadow-soft backdrop-blur dark:border-slate-800 dark:bg-slate-900/80">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="mb-2 text-[11px] font-black uppercase tracking-[0.28em] text-indigo-600 dark:text-indigo-300">AI Scanner Console</p>
                    <h1 class="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-50">AI Data Capture</h1>
                    <p class="mt-2 max-w-2xl text-sm font-medium text-slate-500 dark:text-slate-400">Upload receipts, invoices, and mileage photos in one streamlined scanner workflow.</p>
                </div>
                ...
                <div class="scanner-user-wrap">
                    ...
                    <div class="scanner-toolbar mt-3 flex flex-wrap items-center justify-end gap-3">
        ...
        <div class="tabs mb-6 flex flex-wrap gap-2 rounded-3xl border border-slate-200 bg-white/90 p-2 shadow-soft dark:border-slate-800 dark:bg-slate-900/80">
            <button class="tab-link active inline-flex flex-1 items-center justify-center gap-2 rounded-2xl px-4 py-3 text-xs font-black uppercase tracking-[0.18em] sm:text-sm" onclick="openTab(event, 'receipts')">
                <i class="fas fa-receipt"></i> Receipts
            </button>
            <button class="tab-link inline-flex flex-1 items-center justify-center gap-2 rounded-2xl px-4 py-3 text-xs font-black uppercase tracking-[0.18em] sm:text-sm" onclick="openTab(event, 'mileage')">
                <i class="fas fa-gauge-high"></i> Mileage
            </button>
        </div>

        <div id="receipts" class="tab-content scanner-tab-default">
            <div class="stats-container mb-6 grid grid-cols-2 gap-4 lg:grid-cols-4" id="statsContainer">
                ...
                <div class="stat-box rounded-[24px] border border-slate-200/70 bg-white/90 px-5 py-4 shadow-soft dark:border-slate-800 dark:bg-slate-900/80">
                    <div class="stat-label flex items-center gap-2 text-xs font-black uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400"><i class="fas fa-gas-pump text-emerald-500"></i> Vehicle</div>
            ...
            <div class="scanner-content-grid">
                <div class="scanner-content-main">
                ...
                <div class="scanner-content-side">
                    ...
                    <div class="upload-section rounded-[28px] border border-slate-200/70 bg-white/90 p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900/80">
                        <form id="mileageUploadForm" enctype="multipart/form-data">
                            <div class="form-group mb-5">
                                ...
                                <input class="w-full rounded-2xl border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-900 shadow-sm dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-100" type="text" id="vehicleReg" name="vehicle_reg" placeholder="e.g., 211D12345" value="<?php echo htmlspecialchars($scannerDefaultVehicleReg); ?>" required>
                            </div>
        ...
        <div class="filter-content rounded-[28px] border border-slate-200 bg-white p-7 shadow-hard dark:border-slate-800 dark:bg-slate-900">
            ...
            <div class="filter-group">
                ...
                <select id="filterCategory">
                    ...
                    <option value="Fuel">Vehicle</option>
