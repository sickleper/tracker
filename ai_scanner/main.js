...
const nativeSwalFire = (window.Swal && typeof window.Swal.fire === 'function')
    ? window.Swal.fire.bind(window.Swal)
    : null;

function scannerSwalIsDark() {
    return document.documentElement.classList.contains('dark')
        || document.documentElement.classList.contains('theme-dark');
}

function buildScannerSwalOptions(options = {}) {
    const isDark = scannerSwalIsDark();
    const incomingCustomClass = options.customClass || {};
    ...
    return {
        ...
        ...options,
        customClass: {
            ...scannerSwalBaseClasses,
            ...incomingCustomClass
        }
    };
}

function openScannerSwal(...args) {
    if (!nativeSwalFire) {
        return Promise.reject(new Error('SweetAlert is not available.'));
    }
    ...
    if (typeof args[0] === 'string') {
        ...
        return nativeSwalFire(buildScannerSwalOptions({ title, text, icon }));
    }

    return nativeSwalFire(buildScannerSwalOptions(args[0] || {}));
}

if (window.Swal && !window.Swal.__scannerThemed) {
    window.Swal.__scannerThemed = true;
    window.Swal.fire = (...args) => openScannerSwal(...args);
}
...
function normalizeReviewStatus(status) {
    return status === 'reviewed' ? 'reviewed' : 'pending_review';
}

function normalizeXeroStatus(status) {
    return status || 'not_exported';
}

function renderStatusChip(label, tone) {
    const palette = {
        review_pending: 'background:#fef3c7;color:#92400e;border:1px solid #fcd34d;',
        review_done: 'background:#dcfce7;color:#166534;border:1px solid #86efac;',
        xero_not_exported: 'background:#e2e8f0;color:#334155;border:1px solid #cbd5e1;',
        xero_ready: 'background:#dbeafe;color:#1d4ed8;border:1px solid #93c5fd;',
        xero_exported: 'background:#dcfce7;color:#166534;border:1px solid #86efac;',
        xero_failed: 'background:#fee2e2;color:#991b1b;border:1px solid #fca5a5;',
    };

    return `<span class="badge" style="${palette[tone] || palette.xero_not_exported}">${escapeHtml(label)}</span>`;
}

function renderReviewStatusBadge(status) {
    return normalizeReviewStatus(status) === 'reviewed'
        ? renderStatusChip('Reviewed', 'review_done')
        : renderStatusChip('Pending Review', 'review_pending');
}

function renderXeroStatusBadge(status) {
    const normalized = normalizeXeroStatus(status);
    if (normalized === 'ready_to_export') {
        return renderStatusChip('Ready For Xero', 'xero_ready');
    }
    if (normalized === 'exported') {
        return renderStatusChip('Exported To Xero', 'xero_exported');
    }
    if (normalized === 'failed') {
        return renderStatusChip('Xero Export Failed', 'xero_failed');
    }
    return renderStatusChip('Not Sent To Xero', 'xero_not_exported');
}

function renderReceiptReviewAction(receipt) {
    if (normalizeReviewStatus(receipt.review_status) === 'reviewed') {
        return `<button class="receipt-action-btn view-btn" onclick="markReceiptPending('${receipt.id}')">Move Back To Review</button>`;
    }

    return `<button class="receipt-action-btn view-btn" onclick="markReceiptReviewed('${receipt.id}')">Mark Reviewed</button>`;
}
...
function renderReceipts(receiptsToRender) {
    if (receiptsToRender.length > 0) {
        const orderedReceipts = [...receiptsToRender].sort((a, b) => {
            const aPending = normalizeReviewStatus(a.review_status) === 'pending_review' ? 0 : 1;
            const bPending = normalizeReviewStatus(b.review_status) === 'pending_review' ? 0 : 1;
        ...
        receiptsList.innerHTML = orderedReceipts.map(receipt => `
                            <div class="receipt-card receipt-card-compact" id="receipt-${receipt.id}">
                                <div class="receipt-header receipt-header-compact">
                                    ...
                                    <span class="amount">€${safeMoney(receipt.total_amount)}</span>
                                </div>
                                <div class="receipt-details receipt-details-compact">
                                    <div class="receipt-badge-row">
                                        ...
                                        ${renderReviewStatusBadge(receipt.review_status)}
                                        ${renderXeroStatusBadge(receipt.xero_status)}
                                    </div>
                                    <div class="receipt-confidence"><i class="fas fa-chart-bar"></i> ${(Number.parseFloat(receipt.confidence_score || 0) * 100).toFixed(0)}% confidence</div>
                                </div>
                                <div class="receipt-actions receipt-actions-compact">
                                    ${renderReceiptReviewAction(receipt)}
            ...
            if (data.success) {
                ...
                <div class="modal-section">
                    ...
                    <div class="modal-field">
                        ...
                        <div class="modal-value">${renderReviewStatusBadge(receipt.review_status)}</div>
                    </div>
                    <div class="modal-field">
                        <div class="modal-label">Xero</div>
                        <div class="modal-value">${renderXeroStatusBadge(receipt.xero_status)}</div>
                    </div>
                    ${receipt.review_notes ? `
                    <div class="modal-field">
                        <div class="modal-label">Review Notes</div>
                        <div class="modal-value">${escapeHtml(receipt.review_notes)}</div>
                    </div>
                    ` : ''}
                </div>
                <div class="modal-section">
                ...
                <div class="modal-section modal-section-span modal-section-stack">
                    ...
                    <div class="receipt-actions receipt-actions-compact">
                        ${renderReceiptReviewAction(receipt)}
                        <button class="receipt-action-btn view-btn" onclick="closeModal()">Close</button>
                    </div>
                </div>
            `;
...
async function updateReceiptReview(id, reviewStatus) {
    const isReviewed = reviewStatus === 'reviewed';
    const prompt = await Swal.fire({
        title: isReviewed ? 'Mark receipt as reviewed?' : 'Move receipt back to review?',
        text: isReviewed
            ? 'This will mark the receipt as ready for the Xero review queue.'
            : 'This will put the receipt back into the pending review queue.',
        input: 'textarea',
        inputLabel: 'Review note',
        inputPlaceholder: isReviewed ? 'Optional note for accounting review' : 'Optional reason',
        inputValue: '',
        showCancelButton: true,
        confirmButtonText: isReviewed ? 'Mark Reviewed' : 'Move To Review',
    });
    ...
    try {
        const response = await fetch(`?action=review&id=${encodeURIComponent(id)}`, {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                review_notes: prompt.value || '',
            }),
        });

        const data = await response.json();
        if (!data.success) {
            throw new Error(data.message || data.error || 'Failed to update receipt review.');
        }

        await Swal.fire({
            title: isReviewed ? 'Receipt reviewed' : 'Receipt returned to review',
            text: isReviewed
                ? 'The receipt is now ready for the next accounting step.'
                : 'The receipt is back in the pending review queue.',
            icon: 'success',
        });
        ...
        showError(error.message || 'Failed to update receipt review.');
    }
}

function markReceiptReviewed(id) {
    updateReceiptReview(id, 'reviewed');
}

function markReceiptPending(id) {
    updateReceiptReview(id, 'pending_review');
}
